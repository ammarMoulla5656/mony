...
class dd:
    ...
    def addemplyee(self):
        ...
        # مثال على إضافة موظف جديد إلى قاعدة البيانات
        cursor.execute('''
            INSERT INTO employees (الأسم, domen, monyfirst, deree, year, joinnum, certificate, cheldreen, job_adrees, marital_status, jobhusband)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (self.name, self.domen, self.monyfirst, self.deree, self.year, self.joinnum, self.certificate, self.cheldreen, self.job_adrees, self.marital_status, self.jobhusband))
    ...
    def __init__(self, root):
          ...
          framebutton.grid(row=0, column=0, padx=20, pady=20,sticky='nes')

          but1test = tk.Button(framebutton, width=20, font=("Arial", 19, "bold"), text='test1', highlightthickness=2,height=10)
          but1test.grid(row=0,column=0)

          but2test = tk.Button(framebutton, width=20, font=("Arial", 19, "bold"), text='test1', highlightthickness=2,height=10)
          but2test.grid(row=1,column=0)

          but3test = tk.Button(framebutton, width=20, font=("Arial", 19, "bold"), text='test1', highlightthickness=2,height=10)
          but3test.grid(row=1,column=1)

          but4test = tk.Button(framebutton, width=20, font=("Arial", 19, "bold"), text='test1', highlightthickness=2,height=10)
